...
- 训练比较页指示器缩放改为基于当前过滤后每个计划的显示数据动态计算，移除左右计划共享全局缩放的依赖（重构）。
- 训练比较页缩放计算移除不必要的非空断言，修复分析告警（修复）。
- 训练比较页指示器改为左右计划共享同一缩放，但缩放仅基于当前过滤条件下两侧计划的指标数据动态计算（调整）。
- 训练比较页缩放步进改为按最大值量级自适应，避免小数指标缩放固定为10导致看似不变（调整）。
- 训练比较页在最小指标为0时将Y轴下限扩展为负值，使0落在轴高20%位置以提升对比可读性（调整）。
- 训练比较页缩放范围计算改为保留0值参与最小值判断，确保最小值为0时会触发下限负值扩展（修复）。
- 训练比较页Y轴下限改为动态计算，使最小值在轴高位置不低于20%而非固定对0特殊处理（调整）。
- 训练比较页同步对最大值应用20%留白规则，调整上限使最大值不贴顶（调整）。
- 训练比较页调整布局：指示器移到页面顶部，时间范围卡片移到指示器下方（调整）。
- 训练比较页指示器最大/最小值文本改为带“MAX:”与“MIN:”前缀显示（调整）。
- 训练比较页移除“最后一次记录”指示线与标签，仅保留最大/最小值标记（调整）。
- 训练比较页指示线改为半透明蓝色，并为区间内所有记录值绘制横线但不显示数值（调整）。
- 训练比较页指示线改为不透明蓝色，并将中间记录由横线改为圆点，同值圆点在同高度水平分布避免重叠（调整）。
- 训练比较页中间记录圆点改为半透明蓝色显示（调整）。
- 训练比较页时间范围卡片改为紧凑样式并移除“最近三个月”按钮（调整）。
- 训练比较页指示器将训练名称移到图表下方，并将“更换”文字改为图标按钮提示（调整）。
- 训练比较页移除指示器顶部标题，并将切换图标放到训练名称右侧（调整）。
- 训练比较页指标选择按钮改为“指标”文本加下拉选择样式（调整）。
- 训练比较页指标选择区域改为与时间范围卡片一致的紧凑卡片样式（调整）。
- 训练比较页将指示器的训练名称与切换图标移到图表上方显示（调整）。

## [v0.0.1] 2026.01.30

- 在可视化数据统计页面加入“训练比较 / 趋势变化”两个 Tab，并为各 Tab 提供占位提示内容（结构调整）。
- 新增训练比较页面与筛选/指标计算 Provider，支持时间范围、计划选择与最大/最小/最近值的基础比较展示（功能开发）。
- 修正历史记录页异步弹窗后的 context 使用检查，避免 use_build_context_synchronously 报警（修复）。
- 训练比较页面改用训练记录中的计划名称列表，避免计划库名称不匹配导致无数据显示（修复）。
- 训练比较指示器移除背景与Y轴数值，并将Y轴缩放固定为0-50kg，超过80%%时按10kg步进上调（结构调整）。
- 训练比较卡片移除分区背景色，并将指示器背景改为深灰色（结构调整）。
- 训练比较指示器移除蓝色柱与横线，仅保留最大/最小/最近三条指示线（结构调整）。
- 训练比较指示器调整为最大值黑线、最小值白线、当前值蓝线，并将背景高度改为300（结构调整）。
- 训练比较指示器移除下方指标文字区域，将数值显示在对应指示线上方（结构调整）。
- 训练比较指示器加入数值自动避让逻辑，避免最大/最小/最近标签重叠（结构调整）。
- 训练比较指示器将最小值标签改为线条下方显示，并纳入避让逻辑（结构调整）。
- 训练比较指示器改为自动缩放区间（最大/最小落在20%-80%内，步进10），并为最大/最小区间填充30%%蓝色透明带（结构调整）。
